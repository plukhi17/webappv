...
export class DfftChartComponent implements OnChanges, OnInit {
  ...
  ngOnInit() {   
    console.log('dfftChart.. ngOnInit()');

    if(!this.chart) { // Chart object is empty, then create one.
      this._createChart();
    } 
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    console.log('dfftChart.. ngOnChanges(). changes:',changes);

    // Step 1 - Create chart (When dates are selected)
    if(!this.chart){
      this._createChart();
    } 
  ...
  private _createChart(){
   ...
   if(this.chart) 
     ...
     this.chart = new CanvasJS.Chart('ddftchart', {
  ...
  private _renderNormalInstanceData(){
    console.log('dfftchart._renderNormalInstanceData()');
  ...
  private _renderAbnormalInstanceData(){
    console.log('dfftchart._renderAbnormalInstanceData()');
